...
export default function AddBooks() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const response = await fetch('/api/Urooj/Books/GetAll');
                const data = await response.json();
                setBooks(data);
            } catch (error) {
                console.log(error);
    ...
    async function onSubmit(event: FormEvent<HTMLFormElement>) {
        ...
        if (
            ...
            try {
                const response = await fetch('/api/Urooj/Books/AddBook', {
                    method: 'POST',
                    headers: {
    ...
    const DeleteBook = async (bookId: number) => {
        try {
            const response = await fetch(
                `/api/Urooj/Books/Remove?bookId=${bookId}&ModifiedBy=${username}&id=${id}`,
                {
                    method: 'POST',
                }
            );
    ...
    const updateBook = async (event: FormEvent<HTMLFormElement>) => {
        ...
        if (selectedBook) {
            try {
                const response = await fetch('/api/Urooj/Books/EditBook', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                ...
                if (response.status === 200) {
                    setBooks(
                        books.map((book) =>
                            book.bookId === selectedBook.bookId ? data : book
                        )
                    );
